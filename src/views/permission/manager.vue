<template>
    <div class="app-container">
        ...
        <!-- Form -->
        <el-dialog title='编辑' :visible.sync='editdialogFormVisible' append-to-body :before-close='beforeclose'>
            <el-form :model='adminformState' :rules='rules' ref='ruleForm' label-width='120px' class='demo-ruleForm'>
                ...
                <el-form-item label='权限等级' prop='superior' v-if="ifshowsuperior">
                    <el-select size='small' v-model='adminformState.permissionLevel' placeholder='请选择权限等级'>
                        <el-option v-for='(item,index) in permissionLevel' :key='index' :label='item.label' :value='item.level'>
                        </el-option>
                    </el-select>
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    zhuanyiZiyuan(index, row) {
      var col = this.$children[0].$children[0].$children[1].$children[2];
      console.log( this.$children[0].$children[0].$children[1].$children[2]);
      console.log(col.filterMethod)
  ...
  beforeCreate() {
    ...
    if (!this.$store.getters.zhuanyilist) {
      this.$store.dispatch("getzhuanyiList").then(() => {
